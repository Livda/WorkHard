...
let rec pattern_matching pattern value =
  ...
  | Ml_pattern_int i1, Val_int i2 -> if i1=i2 then [] else raise
  (Error "Pattern didn't match!")
  | Ml_pattern_bool b1, Val_bool b2 -> if b1=b2 then [] else raise
  (Error "Pattern didn't match!")
  | Ml_pattern_cons(e1, e2), Val_cons(v1, v2) -> pattern_matching e1 v1 @
  pattern_matching e2 v2
  | Ml_pattern_pair(e1, e2), Val_pair(v1, v2) -> pattern_matching e1 v1 @
  pattern_matching e2 v2
...
let rec eval env = function
  ...
  | Ml_if(test, exe1, exe2) -> if ((eval env test)=Val_bool true) then
  (eval env exe1) else (eval env exe2)
  ...
  |Ml_letrec(f, exp1, exp2) -> begin
    match exp1 with
    | Ml_fun pattern_list -> let rec env2 = (f, Val_closure(env2, pattern_list))
    :: env in eval env2 exp2
